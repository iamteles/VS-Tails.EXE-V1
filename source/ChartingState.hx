...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	function addNoteUI():Void
	{
		...
		check_eye = new FlxUICheckBox(10, 400, null, null, "Eye Note", 100);
		tab_group_note.add(check_eye);
	...
	private function addNote():Void
	{
		...
		var noteSus = 0;
		var eyeNote = check_eye.checked;

		trace("eyeNote = " + eyeNote);

		_song.notes[curSection].sectionNotes.push([noteStrum, noteData, noteSus, eyeNote, false]);

		curSelectedNote = _song.notes[curSection].sectionNotes[_song.notes[curSection].sectionNotes.length - 1];

		if (FlxG.keys.pressed.CONTROL)
		{
			_song.notes[curSection].sectionNotes.push([noteStrum, (noteData + 4) % 8, noteSus, eyeNote, true]);
		}
